...
## Selesai di codebase
- [x] Patch 0: dokumen root `RENCANA-INTEGRASI-BACKEND-kansor.md` sudah ada.
...
- [x] Patch 3: konfigurasi Laravel server-side sudah ada untuk `services.kansor`, `.env.example`, `PosKantinClient`, `PosKantinException`, cache token service-account, dan test HTTP fake.
...
- [x] Patch 4: dashboard AdminLTE menampilkan status backend dan ringkasan `dashboardSummary`, sidebar dirapikan untuk Transaksi, Simpanan, Pemasok, Pembayaran, Laporan, dan Pengguna.
  Verifikasi lokal 2026-04-27: route `kansor/laporan` aktif, test halaman POS lolos, dan `npm run build` sukses.
...
- [x] Patch 6A: utilitas migrasi data lama sudah ditambahkan di `apps-script/Migration.gs`, action backend `migrateLegacySpreadsheet`, wrapper `PosKantinClient`, env `KANSOR_LEGACY_SPREADSHEET_ID`, dan command `php artisan kansor:migrate-legacy-spreadsheet`.
...
- Patch 6 live wajib diawali preview (`php artisan kansor:migrate-legacy-spreadsheet`) sebelum `--commit`, dan backup source/target perlu dipastikan berhasil atau di-override secara sadar.
