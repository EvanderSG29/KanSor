...
- Backend dari `EvanderSG29/pos-kantin` diambil hanya bagian `apps-script/`, lalu ditempatkan di `KanSor/apps-script/` sebagai source Google Apps Script.
...
|---|---|---|
| Patch 0 | Buat file root `RENCANA-INTEGRASI-BACKEND-POS-KANTIN.md` berisi arsitektur, checklist deploy, patch roadmap, risiko, dan rollback. | Dokumentasi ada, tidak ada secret/URL live/spreadsheet ID. |
| Patch 1 | Copy `pos-kantin/apps-script` ke `KanSor/apps-script`; update `Config.gs` untuk nama database KanSor dan seed admin Evander baru; tambahkan ignore untuk `apps-script/.clasp.json`. | Source backend siap dipush CLASP, tidak membawa frontend Electron/SB Admin dari `pos-kantin/app`. |
