...
test('sync conflict discard and resend write audit logs', function () {
    $syncService = Mockery::mock(KansorSyncService::class);
    ...
    $this->app->instance(KansorSyncService::class, $syncService);
    ...
    expect($discardLog)->not->toBeNull()
        ->and($discardLog?->subject_type)->toBe(KansorSyncConflict::class)
    ...
    expect($resendLog)->not->toBeNull()
        ->and($resendLog?->subject_type)->toBe(KansorSyncConflict::class)
