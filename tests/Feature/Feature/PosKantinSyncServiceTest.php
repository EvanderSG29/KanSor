...
use App\Models\User;
use App\Services\Kansor\KansorLocalStore;
...
test('sync service stores initial pull into local mirrors', function () {
    ...
    KansorDeviceCredential::query()->create([
    ...
    $result = app(KansorSyncService::class)->sync($user, 'manual');

    expect($result['ok'])->toBeTrue()
        ->and(app(KansorLocalStore::class)->payloads($user, 'suppliers'))->toHaveCount(1)
        ->and(app(KansorLocalStore::class)->payloads($user, 'users'))->toHaveCount(1)
        ->and(app(KansorLocalStore::class)->payloads($user, 'foods'))->toHaveCount(1)
...
test('sync service records conflicts from sync push', function () {
    ...
    expect($result['ok'])->toBeTrue()
        ->and(KansorSyncOutbox::query()->first()?->status)->toBe('conflict')
        ->and(KansorSyncConflict::query()->count())->toBe(1);
});
...
test('sync status exposes queued applied failed and latest push summary', function () {
    ...
    KansorSyncConflict::query()->create([
        'scope_owner_user_id' => $user->getKey(),
        'outbox_id' => KansorSyncOutbox::query()
    ...
    $status = app(KansorSyncService::class)->statusForUser($user);
...
test('unresolved conflicts expose field diff context for the sync page', function () {
    ...
    $outbox = KansorSyncOutbox::query()->create([
        'scope_owner_user_id' => $user->getKey(),
    ...
    $conflicts = app(KansorSyncService::class)->unresolvedConflicts($user);
...
test('sync payload columns are stored encrypted for new records', function () {
    ...
    $conflict = KansorSyncConflict::query()->create([
        'scope_owner_user_id' => $user->getKey(),
        'outbox_id' => $outbox->id,
        'entity_type' => 'supplier',
        'entity_remote_id' => 'SUP-006',
...
test('sync payload encryption migration converts legacy plaintext rows', function () {
    ...
    expect($rawOutboxPayload)->toBeString()
        ...
        ->and($rawConflictPayload)->toBeString()
        ->not->toContain('Supplier Plaintext')
        ->and(KansorSyncOutbox::query()->findOrFail($outboxId)->payload)->toMatchArray([
            'id' => 'SUP-007',
            'supplierName' => 'Supplier Plaintext',
        ])
        ->and(KansorSyncConflict::query()->findOrFail($conflictId)->local_payload)->toMatchArray([
            'id' => 'SUP-007',
            'supplierName' => 'Supplier Plaintext',
        ]);
});
