...
test('it can read backend health', function () {
    ...
    $health = app(KansorClient::class)->health();
...
test('it refreshes cached token when backend session expires', function () {
    ...
    $summary = app(KansorClient::class)->dashboardSummary();
...
test('it throws an exception on malformed responses', function () {
    ...
    expect(fn () => app(KansorClient::class)->health())
        ->toThrow(KansorException::class, 'Respons POS Kantin tidak valid');
});
...
test('it throws an exception when backend returns success false', function () {
    ...
    expect(fn () => app(KansorClient::class)->health())
        ->toThrow(KansorException::class, 'Spreadsheet belum dikonfigurasi.');
});
...
test('it wraps connection failures in pos kantin exception', function () {
    ...
    expect(fn () => app(KansorClient::class)->health())
        ->toThrow(KansorException::class, 'Gagal terhubung ke POS Kantin API saat memanggil action [health].');
});
...
test('it throws an exception when backend url is missing', function () {
    ...
    expect(fn () => app(KansorClient::class)->health())
        ->toThrow(KansorException::class, 'POS Kantin API URL belum dikonfigurasi.');
});
...
test('it can request legacy spreadsheet migration preview', function () {
    ...
    $result = app(KansorClient::class)->migrateLegacySpreadsheet([
