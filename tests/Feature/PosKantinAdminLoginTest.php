...
test('it logs in using remote pos kantin credentials and provisions offline access locally', function () {
    ...
    $credential = KansorDeviceCredential::query()->whereBelongsTo($user, 'user')->first();
...
test('it still redirects home when post-login sync fails', function () {
    ...
    $this->mock(KansorSyncService::class, function (MockInterface $mock): void {
        $mock->shouldReceive('sync')
            ->andThrow(new KansorException('Sinkronisasi login gagal.', [
                'category' => 'sync',
            ]));
    });
