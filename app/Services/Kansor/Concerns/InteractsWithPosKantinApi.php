...
namespace App\Services\Kansor\Concerns;
...
trait InteractsWithKansorApi
{
    protected function ensureConfigured(): void
    {
        if (! $this->configured()) {
            throw new KansorException('KanSor API URL belum dikonfigurasi.', [
    ...
    protected function decodePayload(Response $response, string $action): array
    {
        ...
        if (! is_array($payload) || ! array_key_exists('success', $payload)) {
            throw new KansorException('Respons KanSor tidak valid atau bukan JSON yang diharapkan.', [
                'action' => $action,
        ...
        if (($payload['success'] ?? false) !== true) {
            throw new KansorException((string) ($payload['message'] ?? 'KanSor API mengembalikan kegagalan.'), [
                'action' => $action,
