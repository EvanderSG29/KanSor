...
use App\Services\Kansor\Concerns\InteractsWithKansorApi;
use Illuminate\Support\Str;

class KansorClient
{
    use InteractsWithKansorApi;

    public function __construct(
        protected KansorServiceTokenCache $tokenCache,
    ) {}
    ...
    public function loginAsServiceAccount(): string
    {
        ...
        if ($email === '' || $password === '') {
            throw new KansorException('Kredensial admin KanSor belum lengkap di environment.');
        }
        ...
        if (! is_array($data) || ! isset($data['token']) || ! is_string($data['token'])) {
            throw new KansorException('Respons login KanSor tidak valid.', [
                'action' => 'login',
                'data' => $data,
            ]);
        }
    ...
    protected function shouldRefreshToken(KansorException $exception): bool
    {
