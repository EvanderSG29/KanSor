...
const toastStack = document.getElementById('kansor-toast-stack');

const colorClassForToast = (type) => {
    if (type === 'success') {
        return 'bg-success';
    }
...
const showKanSorToast = (message, type = 'info') => {
    if (!toastStack || !message) {
        return;
    }

    const wrapper = document.createElement('div');
    const toastId = `kansor-toast-${Date.now()}-${Math.floor(Math.random() * 1000)}`;

    wrapper.innerHTML = `
        <div id="${toastId}" class="toast text-white ${colorClassForToast(type)} border-0" role="alert" aria-live="assertive" aria-atomic="true" data-delay="4500">
            <div class="d-flex">
    ...
    window.jQuery(toastElement).toast('show');
    window.jQuery(toastElement).on('hidden.bs.toast', () => {
        toastElement.remove();
    });
};

document.querySelectorAll('[data-kansor-toast-message]').forEach((element) => {
    const message = element.getAttribute('data-kansor-toast-message') ?? '';
    const type = element.getAttribute('data-kansor-toast-type') ?? 'info';

    if (message) {
        showKanSorToast(message, type);
    }
});

const confirmModalElement = document.getElementById('kansorConfirmModal');

if (confirmModalElement) {
    const confirmMessageElement = confirmModalElement.querySelector('[data-kansor-confirm-message]');
    const approveButton = confirmModalElement.querySelector('[data-kansor-confirm-approve]');
    ...
    document.querySelectorAll('form[data-confirm-message]').forEach((form) => {
        form.addEventListener('submit', (event) => {
            ...
            if (confirmMessageElement) {
                confirmMessageElement.textContent = form.getAttribute('data-confirm-message') || 'Apakah Anda yakin ingin melanjutkan aksi ini?';
            }

            window.jQuery(confirmModalElement).modal('show');
        });
    });
    ...
    approveButton?.addEventListener('click', () => {
        if (pendingForm) {
            ...
            pendingForm = null;
            window.jQuery(confirmModalElement).modal('hide');
