...
@section('page_header')
    <x-pos.page-header title="Detail Transaksi Harian" subtitle="Lihat ringkasan transaksi, status pembayaran, dan rincian item dalam satu tampilan.">
        <x-slot:actions>
            @can('update', $sale)
                <a href="{{ route('kansor.sales.edit', $sale) }}" class="btn btn-sm btn-outline-primary mr-2">Edit</a>
            @endcan
            @can('delete', $sale)
                <form action="{{ route('kansor.sales.destroy', $sale) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
...
<<<<<<< HEAD:resources/views/kansor/sales/show.blade.php
=======

>>>>>>> 6549984 (	modified:   .env.example):resources/views/pos-kantin/sales/show.blade.php
