...
@section('page_header')
    <x-pos.page-header title="Kelola Pemasok" subtitle="Atur pemasok aktif, kontak, dan potongan dengan struktur tampilan yang sama antar halaman admin.">
        <x-slot:actions>
            <a href="{{ route('kansor.admin.suppliers.create') }}" class="btn btn-primary btn-sm">Tambah pemasok</a>
        </x-slot:actions>
    </x-pos.page-header>
@endsection
...
<div class="card card-outline card-primary">
    ...
    <x-pos.filter-card :action="route('kansor.admin.suppliers.index')" :reset-url="route('kansor.admin.suppliers.index')" title="Filter pemasok" card-class="border-0 shadow-none mb-0">
            <div class="col-md-4">
        ...
        <x-pos.data-table :empty="$suppliers->isEmpty()" :colspan="6" empty-title="Belum ada pemasok" empty-message="Tambahkan pemasok agar menu dan transaksi bisa dikelola sesuai sumbernya.">
            ...
            @foreach ($suppliers as $supplier)
                <tr>
                    ...
                    <td class="text-right">
                        <a href="{{ route('kansor.admin.suppliers.edit', $supplier) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                        <form action="{{ route('kansor.admin.suppliers.destroy', $supplier) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
...
<<<<<<< HEAD:resources/views/kansor/admin/suppliers/index.blade.php
=======

>>>>>>> 6549984 (	modified:   .env.example):resources/views/pos-kantin/admin/suppliers/index.blade.php
