...
@section('page_header')
    <x-pos.page-header title="Kelola Menu / Makanan" subtitle="Jaga katalog makanan tetap rapi, aktif, dan siap dipakai di transaksi harian.">
        <x-slot:actions>
            <a href="{{ route('kansor.admin.foods.create') }}" class="btn btn-primary btn-sm">Tambah makanan</a>
        </x-slot:actions>
    </x-pos.page-header>
@endsection
...
<div class="card card-outline card-primary">
    ...
    <x-pos.filter-card :action="route('kansor.admin.foods.index')" :reset-url="route('kansor.admin.foods.index')" title="Filter menu" card-class="border-0 shadow-none mb-0">
            <div class="col-md-4">
        ...
        <x-pos.data-table :empty="$foods->isEmpty()" :colspan="6" empty-title="Belum ada makanan" empty-message="Tambahkan menu aktif agar petugas bisa memilihnya saat input transaksi.">
            ...
            @foreach ($foods as $food)
                <tr>
                    ...
                    <td class="text-right">
                        <a href="{{ route('kansor.admin.foods.edit', $food) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                        <form action="{{ route('kansor.admin.foods.destroy', $food) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
...
<<<<<<< HEAD:resources/views/kansor/admin/foods/index.blade.php
=======

>>>>>>> 6549984 (	modified:   .env.example):resources/views/pos-kantin/admin/foods/index.blade.php
