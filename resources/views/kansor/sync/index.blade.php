...
@section('page_header')
    <x-pos.page-header title="Status Sinkronisasi" subtitle="Pantau status offline/online, antrean perubahan lokal, dan konflik sinkronisasi.">
        <x-slot:actions>
            <form method="POST" action="{{ route('kansor.sync.run') }}" class="d-inline-block mr-2">
                @csrf
                <button type="submit" class="btn btn-primary btn-sm">Sync sekarang</button>
            </form>
            <form method="POST" action="{{ route('kansor.sync.retry') }}" class="d-inline-block">
                @csrf
                        ...
                        @if ($canResolve)
                            <div class="d-flex flex-column align-items-start">
                                <button
                                    ...
                                    data-action-url="{{ route('kansor.sync.outbox.discard', $conflict['outboxId']) }}"
                                ...
                                <button
                                    ...
                                    data-action-url="{{ route('kansor.sync.outbox.resend', $conflict['outboxId']) }}"
                                    data-entity-label="{{ $conflict['entityLabel'] ?? $conflict['entityType'] }}"
                                    data-remote-id="{{ $conflict['entityRemoteId'] ?: '-' }}"
                                >
                                    Kirim ulang lokal
...
<<<<<<< HEAD:resources/views/kansor/sync/index.blade.php
    <form method="POST" action="{{ route('kansor.sync.run-selected') }}">
=======
    <form method="POST" action="{{ route('pos-kantin.sync.run-selected') }}">
>>>>>>> c97eb59 (Enhance AdminLTE UX with toast/modal and clean obsolete dev notes):resources/views/pos-kantin/sync/index.blade.php
        @csrf
        <div class="card-body table-responsive p-0">
...
>>>>>>> 6549984 (	modified:   .env.example):resources/views/pos-kantin/sync/index.blade.php
